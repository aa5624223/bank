...
$(document).ready(function () {
    ...
    var UserAuth = getUserFunction("借款业务");
    var Tabbuttons = [{
        extend: 'excelHtml5',
        autoFilter: true,
        messageTop: '贷款业务',
    ...
    datatable = $("#tab_dataInfo").DataTable({
        ...
        ajax: {
            url: hosturl + '/Home/Search_BankServer1',
            type: 'POST',
        ...
        "drawCallback": function (settings) {
            ...
            api.column(12, { page: 'current' }).data().each(function (group, i) {
                ...
                var Money2 = api.column(8, { page: 'current' }).data()[i];
                ...
                if (last != group) {
                    $(rows).eq(i).before(
                        ...
                        '<td colspan="5">贷款金额:' + remain[0].toFixed(2) + '，已还：' + remain[1].toFixed(2) + '，待还：' + remain[2].toFixed(2) + '</td>' +
                        '</tr>'
                    );
                    last = group;
                    remain = [0, 0, 0];
                ...
                if (i == Datalength - 1) {
                    if (Datalength!=1) {
                        remain = CalcRemain(remain, Type, Money1, Money2, Money3);
                    }
...
function add_Loan() {
    ...
    if (LoanAmount=="") {
        Alert1("提示", "请填写贷款金额", "btn-danger", "red");
        return;
    }
    //必选项
    if (OccDate=="") {
        Alert1("提示", "请填写贷款日期", "btn-danger", "red");
        return;
    }
    if (EndDate=="") {
    ...
    $.ajax({
        type: 'POST',
        url: hosturl + '/Home/Add_BankServer1',
        data: formData,
        cache: false,
        dataType: 'JSON',
        processData: false,
...
function add_repay() {
    ...
    $.ajax({
        type: 'POST',
        url: hosturl + '/Home/Add_BankServer2',
        data: formData,
        dataType: 'JSON',
        cache: false,
        processData: false,
            ...
            buttons: {
                confirm: {
                    ...
                    action: function () {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: hosturl + '/Home/Del_BankServer1',
                            data: formData,
                            dataType: 'JSON',
                            cache: false,
                            processData: false,
            ...
            buttons: {
                confirm: {
                    ...
                    action: function () {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: hosturl + '/Home/Del_BankServer1_2',
                            data: formData,
                            dataType: 'JSON',
                            cache: false,
                            processData: false,
...
function Edit_BankService1() {
    ...
    $.ajax({
        type: 'POST',
        url: hosturl + '/Home/Edit_BankServer1',
        data: formData,
        dataType: 'JSON',
        cache: false,
        processData: false,
...
function init_Search() {
    ...
    //$("#Ser_Begin,#Ser_End,#End_Begin,#End_End").change(function () {
    //    datatable.draw();
    //})
}
