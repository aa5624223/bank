...
$(document).ready(function () { 
    ...
    datatable = $("#tab_dataInfo").DataTable({
        ...
        drawCallback: function (settings) {
            ...
            api.column(2, { page: 'current' }).data().each(function (group, i) {
                ...
                CalcRemain(remain, Money1, Money2, Money3);
                if (i == Datalength - 1) {
                    $(rows).eq(i).after(
                        "<tr>" +
    ...
    datatable2 = $("#tab_dataInfo2").DataTable({
        ...
        drawCallback: function (settings) {
            var api = this.api();
            var rows = api.rows({ page: 'current' }).nodes();
            var Datalength = api.column(10, { page: 'current' }).data().length;
            var last = "企业:" + api.column(10, { page: 'current' }).data()[0] + ',银行:' + api.column(11, { page: 'current' }).data()[0];
            // 借款金额 5,还款金额 6,余额 7 企业10 银行11
            var remain = [0, 0, 0];
            api.column(10, { page: 'current' }).data().each(function (group, i) {
                ...
                var Bank = api.column(11, { page: 'current' }).data()[i];
                var newStr = "企业:" + group + ",银行:" + Bank
                group = newStr
                if (last != group) {
                    $(rows).eq(i).before(
                        '<tr  style="font-size:140%;font-weight:600;text-align:left">' +
                        '<td colspan="6">' + last + '</td>' +
                        '<td style="text-align:center">合计</td>' +
                        '<td colspan="8">贷款金额:' + remain[0].toFixed(2) + '，已还：' + remain[1].toFixed(2) + '，待还：' + remain[2].toFixed(2) + '</td>' +
                        '</tr>'
                    );
                    last = group;
                    remain = [0, 0, 0];
                ...
                if (i == Datalength - 1) {
                    if (Datalength != 1) {
                        remain = CalcRemain(remain, Money1, Money2, Money3);
                    }
                    $(rows).eq(i).after(
                        '<tr  style="font-size:140%;font-weight:600;text-align:left">' +
                        '<td colspan="6">' + last + '</td>' +
                        '<td style="text-align:center">合计</td>' +
                        '<td colspan="8">承兑金额:' + remain[0].toFixed(2) + '，已还：' + remain[1].toFixed(2) + '，待还：' + remain[2].toFixed(2) + '</td>' +
                        '</tr>'
                    );
                }
                remain = CalcRemain(remain, Money1, Money2, Money3);
...
function CalcRemain(remain,Money1,Money2,Money3) {
    remain[0] += Money1;
    remain[1] += Money2;
    remain[2] += Money3;
    return remain;
    ...
    Vue_Search = new Vue({
        ...
        methods: {
            btn_Search: function () {
                ...
                $.ajax({
                    type: 'POST',
                    url: hosturl + '/Home/Search_ServerRec3',
                    data: formData,
                    dataType: 'JSON',
                    cache: false,
                    processData: false,
            ...
            Search_DrawList: function () {
                ...
                var _this = this;
                var formData = new FormData();
                formData.append("Type", "CreditBusinessInfo");
                $.ajax({
                    type: 'POST',
        ...
        mounted: function () {
            ...
            if (Company != '' && Methods == "0") {
                ...
                today = (now.getFullYear()-1) + "-" + fix(month, 2) + "-" + (fix(now.getDate(), 2));
